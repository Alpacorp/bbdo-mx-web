...
- **Banner propio por caso** (`scripts/build-case-banners.mjs`). Se resuelve en
  tres pasos: `bannerVideo` del frontmatter → `/v/case/<slug>.mp4` por
  convención → el clip compartido. La existencia se comprueba en disco al
  construir, igual que el hermano AV1 del banner del home.
...
│   ├── build-case-banners.mjs   un bucle de banner por caso, desde su still
...
| ~~A4~~ | ~~Las 19 fichas **abrían con la misma cortina roja** y con el mismo metraje~~ — **RESUELTO el 2026-08-31.** Cada ficha abre con su propio vídeo, 128–351 KB, y solo carga el suyo. **No es el film de la campaña**: es un empuje lento sobre el key visual de ese caso, generado por `scripts/build-case-banners.mjs`, porque los films viven en una cuenta de Vimeo a la que no hay acceso. Es material de esa campaña, que el clip compartido nunca fue. Cuando lleguen los cortes reales: `public/v/case/<slug>.mp4` o `bannerVideo` en el frontmatter, sin tocar código. | ~~Video por caso~~ · cortes reales |
