 ...
 * public/site.webmanifest — and src/wordmark.ts, which is the same traced
 * outline as a module the site itself renders. All of them are committed; this
 * script is only run again when the source logo or the tile design changes.
 *
 * src/wordmark.ts is generated rather than hand-copied for the obvious reason:
 * a 2.5 KB path string pasted into a second file is a second file to forget.
 *
...
// The tracer moved to its own module when build-figures.mjs needed it too.
...
/**
 * The same outline, as a module. The header used to render the logo as a PNG
 * and repaint it white with `filter: brightness(0) invert(1)` — a raster
 * forced through a filter to change colour, which also promotes it to its own
 * compositing layer. As a path it takes `currentColor` instead, and it is
 * sharp at any size, including the one the footer wants.
 */
const wordmarkModule = `/**
 * wordmark.ts — the BBDO wordmark, traced.
 *
 * GENERATED by scripts/build-icons.mjs from public/v/logo-bbdo.png, which is
 * the only copy of the logo this repo has. Do not edit by hand: run
 * \`npm run icons\` instead. The tracing, and why the outline is traced rather
 * than redrawn, is documented in that script.
 *
 * Coordinates are fractions of the mark's own width, so \`aspect\` is the height
 * of the viewBox and any size is a matter of scaling.
 */

/** Height of the mark as a fraction of its width. */
export const WORDMARK_ASPECT = ${Number(mark.aspect.toFixed(5))};

/** viewBox for an SVG that draws the path below at its natural proportions. */
export const WORDMARK_VIEWBOX = '0 0 1 ${Number(mark.aspect.toFixed(5))}';

/** The outline. One closed ring: the four letters touch. */
...
await writeFile(path.join(ROOT, 'src/wordmark.ts'), wordmarkModule);
console.log(
  `${'src/wordmark.ts'.padEnd(24)} ${String(Buffer.byteLength(wordmarkModule)).padStart(7)} B`
);
